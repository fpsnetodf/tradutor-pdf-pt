...
output_file = "2_final_com_imagens.pdf"
...
def inserir_imagens(temp_pdf_path, image_list, page_rect, translated_text, doc):
    ...
    for img_index, img in enumerate(image_list):
        ...
        # Ajustar a imagem para manter a proporção
        img_width = pix.width
        img_height = pix.height
        scale_factor = min(page_rect.width / img_width, page_rect.height / img_height)  # Calcular o fator de escala

        # Inserir a imagem com a proporção ajustada
        new_page.insert_image(page_rect, pixmap=pix, width=img_width * scale_factor, height=img_height * scale_factor)
